...
  }
  ...
  // --- OPTIMIZATION: Read only the necessary columns instead of the entire sheet ---
  const dirLastRow = directorySheet.getLastRow();
  const attLastRow = attendanceSheet.getLastRow();
  
  if (dirLastRow < 2 || attLastRow < 2) {
    Logger.log("Info: updateActivityLevels - One of the sheets has no data to process.");
    return;
  }

  const directoryValues = directorySheet.getRange(1, 1, dirLastRow, 20).getValues(); // Read A:T
  const attendanceValues = attendanceSheet.getRange(1, 1, attLastRow, 12).getValues(); // Read A:L

  // Create a map for quick lookups: "PersonID_fullname" -> "Activity Level"
  const attendanceMap = new Map();
  Logger.log("--- Building Attendance Map from '" + attendanceStatsSheetName + "' ---");
  for (let i = 1; i < attendanceValues.length; i++) {
    // --- DATA CLEANING: Normalize whitespace to handle extra spaces or hidden characters ---
    const personId = String(attendanceValues[i][0] || "").trim(); // ATT_PERSON_ID_COL_IDX = 0
    const fullName = String(attendanceValues[i][1] || "").replace(/\s+/g, ' ').trim().toLowerCase(); // ATT_FULL_NAME_COL_IDX = 1
    const activityLevel = attendanceValues[i][11]; // ATT_ACTIVITY_LEVEL_COL_IDX = 11
    if (personId && fullName) {
      const mapKey = `${personId}_${fullName}`;
      attendanceMap.set(mapKey, activityLevel);
    }
  }
  Logger.log(`--- Attendance Map built. Total entries: ${attendanceMap.size} ---`);
...
  Logger.log("--- Processing Directory Sheet to Update Activity Levels ---");
...
    const fullName = String(dirRow[1] || "").replace(/\s+/g, ' ').trim().toLowerCase(); // DIR_FULL_NAME_COL_IDX = 1
...
    let backgroundRow = Array(numColumns).fill(null); // Default background is null (transparent/white)
    ...
    // We log the original name from the directory (dirRow[1]) for easier reading.
    Logger.log(`Processing Row ${i + 1}: Name='${dirRow[1]}', ID='${personId}'. Using lookup key: '${lookupKey}'`);
...
        newActivityValue = attendanceMap.get(lookupKey);
        Logger.log(`  -> SUCCESS: Found in Attendance Map. New Activity Level will be '${newActivityValue}'.`);
      } else {
...
        backgroundRow = Array(numColumns).fill(LIGHT_GRAY);
        Logger.log(`  -> FAILED: Not found in Attendance Map. Setting to 'Archive' and highlighting row.`);
      }
    } else {
        Logger.log(`  -> SKIPPED: Missing Person ID or Full Name in this Directory row.`);
    }
...
  }
  Logger.log("--- Finished Processing Directory Sheet ---");
...
  }
  ...
  // --- OPTIMIZATION: Read only the necessary columns instead of the entire sheet ---
  const dirLastRow = directorySheet.getLastRow();
  const donorLastRow = donorStatsSheet.getLastRow();

  if (dirLastRow < 2 || donorLastRow < 2) {
    Logger.log("Info: updateGivingLevels - One of the sheets has no data to process.");
    return;
  }

  const directoryValues = directorySheet.getRange(1, 1, dirLastRow, 19).getValues(); // Read A:S
  const donorStatsValues = donorStatsSheet.getRange(1, 1, donorLastRow, 10).getValues(); // Read A:J
...
    const firstName = String(donorStatsValues[i][1] || "").replace(/\s+/g, ' ').trim().toLowerCase(); // DS_MATCH_FIRST_NAME_COL_IDX = 1
    const lastName = String(donorStatsValues[i][2] || "").replace(/\s+/g, ' ').trim().toLowerCase(); // DS_MATCH_LAST_NAME_COL_IDX = 2
...
      const lastNameDir = String(dirRow[2] || "").replace(/\s+/g, ' ').trim().toLowerCase(); // DIR_FOR_DONOR_MATCH_LAST_NAME_COL_IDX = 2
      const firstNameDir = String(dirRow[3] || "").replace(/\s+/g, ' ').trim().toLowerCase(); // DIR_FOR_DONOR_MATCH_FIRST_NAME_COL_IDX = 3
